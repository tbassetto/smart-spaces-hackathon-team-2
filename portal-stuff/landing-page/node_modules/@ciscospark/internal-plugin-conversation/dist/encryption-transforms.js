...
var transforms = exports.transforms = (0, _toArray2.default)('outbound', {
  ...
  encryptConversation: function encryptConversation(ctx, key, conversation) {
    ...
    return _promise2.default.resolve(key || ctx.spark.internal.encryption.kms.createUnboundKeys({ count: 1 })).then(function (keys) {
  ...
  maybeEncryptTarget: function maybeEncryptTarget(ctx, key, activity) {
    ...
    return ctx.spark.internal.conversation.get({ url: conversationUrl }).then(function (conversation) {
      if (!conversation.defaultActivityEncryptionKeyUrl) {
        return ctx.spark.internal.conversation.updateKey(conversation).then(function (updateKeyActivity) {
  ...
  encryptPropScr: function encryptPropScr(ctx, key, object) {
    ...
    return ctx.spark.internal.encryption.encryptScr(key, object.scr).then(function (scr) {
      object.scr = scr;
    });
  },
  encryptTextProp: function encryptTextProp(ctx, name, key, object) {
    ...
    return ctx.spark.internal.encryption.encryptText(key.uri || key, object[name]).then(function (ciphertext) {
